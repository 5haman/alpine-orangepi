...
static const std::string aws_accesskeyid       = "S3FS_ACCESSKEY";
...
static int get_access_keys(void)
{
  ...
  char* S3FS_ACCESSKEY     = getenv("S3FS_ACCESSKEY");
  char* S3FS_SECRETKEY = getenv("S3FS_SECRETKEY");
  if(S3FS_ACCESSKEY != NULL || S3FS_SECRETKEY != NULL){
    if( (S3FS_ACCESSKEY == NULL && S3FS_SECRETKEY != NULL) ||
        (S3FS_ACCESSKEY != NULL && S3FS_SECRETKEY == NULL) ){
      S3FS_PRN_EXIT("if environment variable S3FS_ACCESSKEY is set then S3FS_SECRETKEY must be set too.");
      return EXIT_FAILURE;
    }
    if(!S3fsCurl::SetAccessKey(S3FS_ACCESSKEY, S3FS_SECRETKEY)){
